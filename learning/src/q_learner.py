...
class QLearner:
    ...
    def __init__(self, env, sourceQ_file=None,
                 ...
                 init_e=1.0, final_e=0.1, exploration_anneal_episodes=150000):
        self.env = env.env
    ...
    def flatten_state(self, state):
        ...
        for i in range(len(state)):
            index = self.feature_bins[i].index(min(self.feature_bins[i], key=lambda x:abs(state[i]-x)))
            digitized_state.append(self.feature_bins[i][index])
            flat_state = flat_state + total_values * (index)
    ...
    def select_action(self, state):
        ...
        # print("POSSIBLE ACTIONS: (0=forward, 1=left, 2=right)")
        # action = input()
        # return action
