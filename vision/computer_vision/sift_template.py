...
def cd_template_matching(img, template):
	...
	for scale in np.linspace(1.5, .5, 50):
		...
		matches = cv2.matchTemplate(img_canny, resized_template, cv2.TM_CCORR_NORMED)
		miv, mav, mil, mal = cv2.minMaxLoc(matches)
		if mav > best_value:
			best_match = (mal, (mal[0] + resized_template.shape[1], mal[1] + resized_template.shape[0]))
			best_value = mav
